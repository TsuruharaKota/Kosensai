...
int main(int argc, char **argv) {
  ...
  ros::NodeHandle n;
  ros::Publisher gyro_yaw_pub = n.advertise<std_msgs::Float64>("gyro_yaw", 10);
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    int16_t gzRaw = i2c_read_byte_data(pi, handle_datum, 0X47) << 8 |
                    i2c_read_byte_data(pi, handle_datum, 0x48);
  ...
  while (ros::ok()) {
    ...
    yaw.data = (double)degree + ((double)function_a * (double)loops_number);
    //        cout << time_diff << endl;
